...
function RecentImpactPanel({ impact }: { impact: ChangeImpact }) {
  ...
  return (
    <section className="change-impact-card p-5 sm:p-7" aria-labelledby="recent-impact-title">
      ...
      {inputChanges.length ? (
        <div className="mt-5 flex flex-wrap gap-2">
          {inputChanges.map(({ label, previous, next }, index) => (
            <span key={label} className={`inline-flex items-center gap-2 rounded-lg border border-border bg-white px-3.5 py-2 font-semibold text-text-primary ${index === 0 ? "text-lg sm:text-xl" : "text-sm"}`}>
              <span className="text-xs uppercase tracking-[0.12em] text-text-muted">{label}</span>
    ...
    <section className="product-hero" aria-labelledby="diagnosis-target-title">
      <div className="grid lg:grid-cols-[minmax(0,1.45fr)_minmax(19rem,.75fr)]">
        <div className="p-5 sm:p-7 lg:p-8">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan">Your target university</p>
          <h1 id="diagnosis-target-title" className="mt-3 max-w-4xl break-words text-3xl font-bold leading-tight text-white sm:text-4xl lg:text-5xl">{program.universityName}</h1>
          <p className="mt-2 break-words text-lg font-semibold text-text-on-dark-muted sm:text-xl">{program.programName}</p>
