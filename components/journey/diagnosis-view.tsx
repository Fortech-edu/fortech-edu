    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <DiagnosisCard
          ...
          tone="positive"
          emptyLabel="Add a few more profile details to see what already works in your favour."
        />
        <DiagnosisCard
          title="Gaps & actions"
          ...
          tone="action"
          emptyLabel="No gaps found. Nothing in your profile needs fixing before you explore programs."
        />
        <DiagnosisCard
          title="Unknown"
          ...
          tone="unknown"
          emptyLabel="Nothing is unknown. Every field we use for matching has a value."
        />
      </div>
...
function DiagnosisCard({
  ...
  /** Shown instead of the list. States the result, so an empty card never reads as "still loading". */
  emptyLabel: string;
}) {
    ...
    <section className="rounded-3xl border border-forest-100 bg-white p-5 sm:p-6">
      ...
      <ul className="mt-5 space-y-3">
        {items.length > 0 ? (
          ...
          <li className="flex gap-3 text-sm leading-6 text-muted">
            <span className={`mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${markerClass}`} aria-hidden="true">
              ✓
            </span>
